...
- (void)apiManager {
    ...
    [YZHttpApiManager apiMineAppointDetailWithParameters:@{@"id": _appoint.ID}
                                       completionHandler:^(id response, NSError *error) {
                                           ...
                                           _dicEmployee = [response valueForKeyPath:@"data.orderData.employee"];
#pragma mark - 最新修改图片数组
                                           _arrFile     = [response valueForKeyPath:@"data.orderData.fileViewList"];
                                           _arrMessage  = [response valueForKeyPath:@"data.orderData.messageList"];
                                           _appoint     = (YZAppointModel *)[[YZAppointModel alloc] initWithDict:[response valueForKeyPath:@"data.orderData"]];
    ...
    if ([_arrSection[indexPath.section] isEqualToString:@"section_appoint"]) {
        switch (indexPath.row) {
            ...
            case 5: {
                ...
                [(UILabel *)[cell viewWithTag:1] setText:[NSString stringWithFormat:@"%@", _appoint.commissionRate]];
                break;
            }
            case 6: {
            ...
            case 7: {
                ...
                [(UILabel *)[cell viewWithTag:1] setText:[NSString stringWithFormat:@"%@", _appoint.fundManager?:@""]];
                break;
            }
            default:
                break;
        ...
        switch (indexPath.row) {
            case 0: {
                ...
                [(UILabel *)[cell viewWithTag:1] setText:_appoint.customerName];
                break;
            }
            case 1: {
                cell = [tableView dequeueReusableCellWithIdentifier:@"cell_idNo"];
                [(UILabel *)[cell viewWithTag:1] setText:_appoint.identifyNo];
                break;
            }
            default:
                break;
