...
+ (void)updateUserInfoWithVC:(UIViewController *)vc toastText:(NSString *)text finish:(void(^)())finishBlack{
    NSDictionary *dic = @{@"nickname"   : yz_user.nickname,
                          @"email"      : yz_user.email,
                          @"gender"     : @(yz_user.gender),
                          @"address"    : yz_user.address,
                          @"vocation"   : yz_user.vocation,
                          @"description": yz_user.descriptions,
                          @"age"        : @(0)};
...
+ (void)saveCookies {
    ...
    [defaults setObject:cookiesData forKey:KLoginCookie];
    [defaults synchronize];
}
+ (void)loadCookies {
    NSArray *cookies = [NSKeyedUnarchiver unarchiveObjectWithData:[[NSUserDefaults standardUserDefaults] objectForKey:KLoginCookie]];
