...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 0) {
        ...
        if (self.comeType == YZEditorAddressComeTypeEditor) {
            ...
            self.numberTF.text = self.model.receivePhone;
        }
        return cell;
...
- (void)saveAndUseAction {
    ...
    if (self.comeType == YZEditorAddressComeTypeEditor) {
        [YZHttpApiManager apiMineContractEditorAddressWithParameters:@{@"receiveName": self.nameTF.text, @"receivePhone": self.numberTF.text, @"area": self.addressLbl.text, @"receiveAddress": self.detailTF.text, @"isDefault": self.defaultSW.isOn ? @1 : @0, @"id": self.model.ID} completionHandler:^(id response, NSError *error) {
