...
- (void)createViewWithArray:(NSArray *)array number:(NSArray *)numberArray{
    ...
    for (int i = 0; i < count; i++) {
        ...
        }else if ([but.lbl.text isEqualToString:@"合同申请"]) {
            but.titleImage.image = [UIImage imageNamed:@"contract_icon"];
        }else {
//
            but.titleImage.image = [UIImage imageNamed:@"personquestion_icon2"];
    ...
    for (int i = 0; i < rows - 1; i++) {
        if (i == 0) {
            UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake(0, height * (i + 1) + 1, S_W, 3)];
            lineView.backgroundColor = YZ_GRAY_COLOR;
            [self.backView addSubview:lineView];
        }else{
