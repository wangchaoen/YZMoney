...
- (void)viewDidLoad {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (self.comeType != FPCertificationComeTypeWithNot) {
            self.nameTF.text = yz_user.verifyName;
            self.companyTF.text = yz_user.verifyCompanyName;
            self.numberTF.text = yz_user.verifyIdentityNo;
            [self.imageArray addObject:yz_user.verifyBusinessCard?:@""];
            ...
            if (self.comeType == FPCertificationComeTypeWithGoing
                || self.comeType == FPCertificationComeTypeWithSuccess) {
